<script>
  ...
  function buildGISUrls(settings) {
    ...
    // Process boundary URL
    const rawBoundary = settings.boundary;
    if (rawBoundary) {
      urls.boundary = rawBoundary.startsWith('http') 
        ? rawBoundary 
        : `${GIS_BASE_URL}/${rawBoundary}.json`;
    }

    // Process place for places URL
    let placeValue = settings.place || '';
    if (!placeValue) {
      placeValue = urls.boundary?.includes('world_110m.json') ? 'world' : 'au';
    }
    if (placeValue) {
    ...
    // Add optional URLs
    if (settings.overlay) urls.overlay = settings.overlay;
    if (settings.basemap) urls.basemap = settings.basemap;
